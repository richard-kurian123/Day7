...
public sameCeo(){
Object.keys(this.secondObj).forEach(tenant => {
  const { CEO, name } = this.secondObj[tenant];
  if (this.sameCeoResults[CEO]) {
    this.sameCeoResults[CEO] = [{ tenant, name }];
  } else {
    this.sameCeoResults[CEO].push({ tenant, name });
  }
});
